...
const Expense = () => {
  ...
  useEffect(() => {
    const total = expenses.filter(exp => exp.category !== 'Emergency').reduce((sum, exp) => sum + exp.amount, 0);
  ...
  const replaceNumberWords = (text) => {
    ...
    for (const [word, number] of Object.entries(numberWords)) {
      processedText = processedText.replace(new RegExp(`\\b${word}\\b`, 'g'), number);
    }
    return processedText;
  };

  const classifyWithBackend = async (text) => {
    try {
      const token = localStorage.getItem('token');
      const res = await axios.post('http://localhost:5000/api/expenses/classify-category', { description: text }, {
        headers: { Authorization: `Bearer ${token}` }
      });
      return res.data.category;
    } catch (err) {
      console.error('Classification API failed, falling back to local');
      return classifyCategoryLocal(text);
    }
  };

  const classifyCategoryLocal = (text) => {
    const categoryMap = {
    ...
    // Priority to Emergency keywords
    if (['health', 'medicine', 'doctor', 'hospital', 'pharmacy', 'medical','emergency', 'medical emergency', 'family function', 'wedding', 'funeral', 'hospital emergency', 'accident'].some(kw => lowerText.includes(kw))) {
      return 'Emergency';
    }
  ...
  const parseTranscript = async (text) => {
    // Preprocess text to convert number words to digits
    const processedText = replaceNumberWords(text);

    let amountMatch = processedText.match(/(?:rupees?|rs\.?|₹)\s*(\d+(?:\.\d{1,2})?)/i);
    if (!amountMatch) amountMatch = processedText.match(/\$(\d+(?:\.\d{1,2})?)/);
    if (!amountMatch) amountMatch = processedText.match(/\b(\d+(?:\.\d{1,2})?)\s*(?:dollars?|rupees?)/i);
    if (!amountMatch) amountMatch = processedText.match(/\b(\d+(?:\.\d{1,2})?)\b/);

    const amount = amountMatch ? parseFloat(amountMatch[1]) : '';

    const category = await classifyWithBackend(text);
                  ...
                  .map(expense => (
                    <div key={expense._id} className={`expense-item ${editingId === expense._id ? 'editing' : ''}`}>
                      <div className="expense-info">
                        <span className={`expense-category ${expense.category === 'Emergency' ? 'emergency' : ''}`}>{expense.category}</span>

...
const { predictEmergency } = require("../services/MLTrainer");
...
exports.addExpense = async (req, res) => {
  try {
    ...
    // Calculate total expenses for current month (EXCLUDE Emergency)
  ...
  // ML Prediction integration (rule-based, based only on user-entered amount/category)
  const predictedType = await predictEmergency({ category, amount: parseFloat(amount) });
...
exports.getSummary = async (req, res) => {
  try {
    ...
    const totalExpenses = expenses.filter(exp => exp.category !== 'Emergency').reduce((sum, exp) => sum + exp.amount, 0);
    const thisMonth = new Date();
    thisMonth.setDate(1);
    const thisMonthExpenses = expenses.filter(exp => new Date(exp.date) >= thisMonth && exp.category !== 'Emergency').reduce((sum, exp) => sum + exp.amount, 0);
    ...
    res.json({
      stats: {
        ...
        hasSalary: currentMonthSalary > 0
      },
      recentExpenses
    });
  } catch (error) {
...
exports.classifyCategory = async (req, res) => {
  try {
    const { description } = req.body;
    if (!description) {
      return res.status(400).json({ message: "Description is required", category: "Other" });
    }
    const category = classifyCategory(description);
    res.json({ category });
  } catch (error) {
    console.error('Error classifying category:', error);
    res.status(500).json({ message: "Classification error", category: "Other" });
  }
};
